...
function UserProfileImage(props) {
	...
	const { updateUserPosts, handleUpdateAlgoliaUser } = Store();
	...
	const handleUploadAvatar = (error, file) => {
		...
		storage
			.ref(`user_avatars/${uploadedFile.name}`)
			.put(uploadedFile)
	...
	// Some info
	const isUserOwner =
		(loggedUser != "no user" && loggedUser.id == profileUser?.id) ||
