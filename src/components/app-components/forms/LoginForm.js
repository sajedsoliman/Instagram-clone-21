...
function LoginForm({ handleLoginModalClose }) {
	...
	const { values: user, handleInputsChange, validationErrors } = useForm(userInitialValues, false);
	...
	const handleSignInWithPasswordAndEmail = (e) => {
		e.preventDefault();
		...
		<IF condition={!googleLastStep} elseChildren={googleLastStepRender}>
			<div>
				<UserForm
					...
					user={user}
					actionHandler={handleSignInWithPasswordAndEmail}
					handleChange={handleInputsChange}
					action={"login"}
				/>

				<Button
					className={classes.googleSignUpBtn}
					fullWidth
