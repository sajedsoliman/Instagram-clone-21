...
export default function Nav({ navClassName }) {
	...
	const handleToggleNotifications = () => {
		...
		if (windowWidth < 600) history.replace("/accounts/activity", { notifications });
		else
			...
			{user != "no user" ? (
				<div className={classes.secondaryLinks}>
					...
					<Link ref={activityLinkRef} onClick={handleToggleNotifications}>
						<Badge variant="dot" color="secondary" invisible={!isThereUnseenNoti} overlap="circle">
							<IF condition={showNotifications} elseChildren={<FavoriteBorderIcon />}>
								<Favorite />
							</IF>
						</Badge>
					</Link>
					<Link underline="none" onClick={handleAnchorEl} aria-haspopup="true">
						<Avatar
							className={clsx(classes.userAvatar, { active: anchorEl })}
							src={user?.avatar}
							alt={user.fullName[0]}
						>
							{user.fullName[0]}
